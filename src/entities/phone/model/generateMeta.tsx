...
export const generateMeta = (phone: Phone) => {
  ...
  const description =
    `${phone.model} ${phone.storage}GB с ${phone.ram}GB RAM. ${phone.color}. Лучшая цена — ${phone.price} ₽.` +
    (phone.warrantyMonths ? ` Гарантия ${phone.warrantyMonths} мес.` : "") +
    ` ${
      phone.inStock
        ? "Есть в наличии — доставка прямо сейчас!"
        : "Нет в наличии — уточняйте дату поступления."
    }`
