...
export const PhoneImageSchema = z.object({
  ...
  alt: z
    .string()
    .min(1, "Альтернативный текст изображения не может быть пустым."),
})
...
export const PhoneSchema = z.object({
  ...
  storage: z
    .number()
    .int()
    .positive("Объем памяти должен быть положительным целым числом."),
  ram: z
    .number()
    .int()
    .positive("Объем RAM должен быть положительным целым числом."),
  ...
  releaseDate: z
    ...
    .regex(
      DD_MM_YYYY_REGEX,
      "Дата выпуска должна быть в формате dd.mm.гггг (например, 17.01.2024)."
    )
    .refine((dateString) => {
  ...
  warrantyMonths: z
    .number()
    .int()
    .positive("Гарантия должна быть положительным целым числом.")
    .optional(),
