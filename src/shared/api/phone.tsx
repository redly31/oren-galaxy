...
import { z } from "zod"
import type { Phone } from "../model/Phone"
...
export async function getPhones(): Promise<Phone[]> {
  ...
  try {
    ...
    if (error instanceof z.ZodError) {
      console.error(
        "Ошибка валидации данных при получении списка телефонов:",
        error.issues
      )
...
export async function getPhone(id: string): Promise<Phone> {
  ...
  try {
    ...
    if (error instanceof z.ZodError) {
      console.error(
        `Ошибка валидации данных при получении телефона ${id}:`,
        error.issues
      )
      throw new Error(
        `Получены некорректные данные для телефона с ID ${id} от API.`
      )
    }
    throw error
  }
